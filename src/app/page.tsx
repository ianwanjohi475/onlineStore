...
export default async function HomePage() {
  const [flash, bestSellers, newArrivals, earbuds, speakers, powerBanks, chargers] = await Promise.all([
    getFlashSale(), getBestSellers(), getNewArrivals(),
    getByCategory("earbuds"), getByCategory("speakers"), getByCategory("power-banks"), getByCategory("chargers"),
  ]);
  const topDeals = [...flash]
    .sort((a, b) => discountPercent(b.compareAt!, b.price) - discountPercent(a.compareAt!, a.price))
    .slice(0, 10);
  const audio = [...earbuds, ...speakers].slice(0, 10);
  const power = [...powerBanks, ...chargers].slice(0, 10);
